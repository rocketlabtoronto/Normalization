"""
SEC Quarterly Filing Download Tool
Downloads and saves the latest 10-Q filings for individual companies by CIK.

This script focuses on:
- Downloading and saving the latest 10-Q filing to organized folders
- Creating filing storage system for reuse across pipeline steps (Step 1.5x → Step 2)
- Building permanent file structure with proper naming conventions
- Only downloading if the latest 10-Q has not already been saved

Usage:
python 1.51_Download10Q.py --cik 0000123456 [--symbol TICK --exchange NASDAQ]

Output: 
- 10-Q filings saved to sec_filings/10Q/ folder
- Investigation report saved to staging/cik_{cik}_10q_download.json
"""
...
def extract_latest_10q(submissions: Dict) -> Optional[Dict]:
    """
    Finds the most recent 10-Q filing from SEC submissions.
    Only looks for original 10-Q forms, excludes amendments (10-Q/A).
    """
    ...
    for form, acc, fdate, pdoc in zip(forms, acc_nums, dates, primary_docs):
        if form == "10-Q" and fdate:  # Only original 10-Q, exclude amendments
            try:
...
def fetch_filing_text(cik: str, accession_number: str, primary_document: str, form_type: str = "10-Q") -> Optional[str]:
    """
    Downloads the actual text content of a specific SEC filing document and saves it to organized folders by form type.
    Creates permanent file storage that can be reused across pipeline steps.
    Only downloads when the file is not already present in storage.
    
    Args:
        cik: Company CIK number
        accession_number: SEC accession number (kept in original format with dashes)
        primary_document: Primary document filename
        form_type: Type of form (10-Q original filing) for folder organization
    """
    # Normalize CIK for consistent naming
    cik_padded = str(cik).zfill(10)
    acc_clean = accession_number.replace('-', '')  # Only for URL construction
    
    # Create organized folder structure by form type (e.g., 10Q)
    form_folder = form_type.replace('-', '').replace('/', '_')
...
"""
    ...
    # Check if file already exists in permanent storage; if so, do not re-download
    ...
    # Download from SEC Archives (URL uses cleaned accession number)
...
def analyze_cik(cik: str, symbol: Optional[str] = None, exchange: Optional[str] = None) -> Dict:
    """
    Downloads the latest 10-Q filing for a company by CIK.
    Creates permanent file storage for reuse across pipeline steps.
    Skips download if the latest 10-Q is already saved.
    """
    try:
        print(f"[INFO] Downloading latest 10-Q filing for CIK: {cik}")
    except UnicodeEncodeError:
        print(f"Downloading latest 10-Q filing for CIK: {cik}")
    ...
    latest_10q = extract_latest_10q(subs)
    
    if not latest_10q:
        report["error"] = "No 10-Q filings found"
        return report

    # Download the 10-Q filing
    form_type = latest_10q.get("form", "10-Q")
    accession = latest_10q.get("accessionNumber")
    primary_doc = latest_10q.get("primaryDocument")
    
    entry = {
        "form": form_type, 
        "filingDate": latest_10q.get("filingDate"), 
        "accessionNumber": accession, 
        "primaryDocument": primary_doc, 
        "link": build_filing_archives_url(cik, accession, primary_doc)
    }
    
    # Download and save the filing (or reuse stored copy)
    ...
    outname = f"staging/cik_{str(cik).zfill(10)}_10q_download.json"
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python 1.51_Download10Q.py --cik 0000123456 [--symbol TICK --exchange NASDAQ]")
        sys.exit(1)
    ...
    if "error" in result:
        ...
        if result.get("filing"):
            ...
            if filing.get('downloaded'):
                print(f"  [SUCCESS] Downloaded/Reused: {filing.get('size_kb')} KB")
            else:
    ...
    print(f"\n[INFO] Report saved to: staging/cik_{str(cik).zfill(10)}_10q_download.json")
