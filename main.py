...
class PipelineController:
    ...
    def check_prerequisites(self, skip_steps: List = None) -> bool:
        ...
        required_files = ["DualClassList_19Aug2025.csv"]
        missing_files = []
        ...
        # AI is needed for: Step 3 (always), Step 1.75 (if --ai-check), or general ai_check flag
        ai_steps_to_run = []
        if 3 not in skip_steps:
            ai_steps_to_run.append(3)  # Step 3 always needs AI
        if self.ai_check and 1.75 not in skip_steps:
            ai_steps_to_run.append(1.75)  # Step 1.75 needs AI if --ai-check
            
        openai_required = bool(ai_steps_to_run)
        ...
        if openai_required:
            if not os.getenv("OPENAI_API_KEY"):
                ...
                if 3 in ai_steps_to_run:
                    self.log("Note: OpenAI API key is required for Step 3 (Economic Weight Analysis)", "INFO")
                if 1.75 in ai_steps_to_run:
                    self.log("Note: OpenAI API key is required for AI investigation in Step 1.75", "INFO")
                return False
        else:
            self.log("OpenAI API key not required for selected steps", "INFO")
    ...
    def run_step_2(self) -> bool:
        """Run Step 2: Ticker-to-Share-Class Mapping"""
        self.log("STEP 2: Ticker-to-Share-Class Mapping", "INFO")
        ...
        if not _get_valid_openai_key():
            ...
            return False
        try:
            # Stream subprocess output in real time (unbuffered)
            env = os.environ.copy()
            ...
            with subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=env, text=True, bufsize=1, encoding='utf-8', errors='replace') as proc:
                ...
                ret = proc.wait()
                if ret != 0:
                    self.log(f"Step 2 failed with return code {ret}", "ERROR")
                    return False
            self.log("✅ Ticker-to-Share-Class Mapping completed", "INFO")
            return True
        except Exception as e:
    ...
    def run_pipeline(self, skip_steps: List = None, resume_from: int = 1) -> bool:
        ...
        step_order = [1, 1.75, 2, 3]
        
        # Filter steps based on resume_from and skip_steps
        steps_to_run = [s for s in step_order if s >= resume_from and s not in skip_steps]
        ...
        self.log(f"✅ Completed steps: {sorted(self.completed_steps)}")
        if self.failed_steps:
            self.log(f"❌ Failed steps: {sorted(self.failed_steps)}")
