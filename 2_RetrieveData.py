...
def extract_with_openai(extraction_10k: Dict[str, Any], extraction_10q: Optional[Dict[str, Any]] = None) -> Tuple[List[Dict[str, Any]], float]:
    ...
    prompt = (
        ...
        "prefunded_warrants (object or null).\n\n"
        ...
        "- PREFUNDED WARRANTS: Detect mentions of pre-funded warrants (e.g., 'pre-funded warrants', 'prefunded warrants', 'pre funded warrants').\n"
        "  • If the text indicates pre-funded warrants are exercisable into a specific class, add a property prefunded_warrants on that class.\n"
        "  • Do NOT include pre-funded warrants in shares_outstanding if the text says the reported shares exclude them (e.g., 'This number does not include ... shares issuable upon the exercise of pre-funded warrants ...').\n"
        "  • Apply unit scaling to the warrant counts if unit qualifiers are present near those figures.\n"
        "  • Attach the warrants to the correct equity class they convert into; if unclear, choose the best interpretation consistent with the disclosures (usually common stock).\n"
        "- FORMATTING: After applying any necessary scaling, format shares_outstanding, authorized_shares, and prefunded_warrants.count as strings using U.S. thousands separators (e.g., '549,964,000' or '1,900,000,000'). If a value is unknown or not disclosed, use null\n"
